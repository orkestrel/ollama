...
An Agent runs against a real model on `localhost` with no cloud dependency, a small local model, and no API key. The design is deliberately spare — an external boundary kept honest. Every `unknown` wire value is narrowed through the `@orkestrel/contract` guards (`isRecord`, `isString`, `isNumber`) rather than a type assertion, and a missing or malformed field degrades to a sensible default (empty content, no usage, `{}` arguments) rather than a throw.
...
| -------------------------- | --------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createOllama`             | function  |                                                                                      | Creates a local Ollama inference provider — a `ProviderInterface` over the daemon's `POST /api/chat`, supporting non-streaming `generate` and streaming `stream`.           |
| `OllamaProvider`           | class     |                                                                                      | Implements the local Ollama inference boundary — a `ProviderInterface` over Ollama's `POST /api/chat`, both non-streaming (`generate`) and streaming NDJSON (`stream`).     |
...
| `DEFAULT_OLLAMA_URL`       | const     |                                                                                      | Names the local Ollama daemon base URL assumed when `OllamaOptions.url` is omitted, `'http://localhost:11434'`.                                                             |
| `DEFAULT_KEEP_ALIVE`       | const     |                                                                                      | Names how long the model stays resident after a call when `OllamaOptions.keepAlive` is omitted, `'5m'` — Ollama's own `keep_alive` default, expressed as a duration string. |
| `DEFAULT_PROVIDER_TIMEOUT` | const     |                                                                                      | Names the per-call deadline in milliseconds when `OllamaOptions.timeout` is omitted, `120_000` — generous enough that a cold model load does not trip it.                   |
...
| `OllamaHTTPError`          | class     |                                                                                      | Represents an error thrown when the Ollama `/api/chat` HTTP transport fails.                                                                                                |
| `isOllamaHTTPError`        | function  |                                                                                      | Checks whether a value is an `OllamaHTTPError`.                                                                                                                             |
...
| `MAX_ERROR_BODY_LENGTH`    | const     |                                                                                      | Names the cap, in characters, on how much of a non-OK response body is incorporated into a thrown `OllamaHTTPError`'s message, `2048`.                                      |
| `mapMessages`              | function  |                                                                                      | Maps conversation turns onto the `/api/chat` wire's minimal message shape.                                                                                                  |
| `buildResult`              | function  |                                                                                      | Builds a `ProviderResult` from a turn's content, reasoning, tool calls, and usage.                                                                                          |
| `parseBody`                | function  |                                                                                      | Parses a non-stream `/api/chat` response body into a wire record.                                                                                                           |
| `extractContent`           | function  |                                                                                      | Extracts the assistant text of one wire record.                                                                                                                             |
| `extractThinking`          | function  |                                                                                      | Extracts the daemon-side reasoning of one wire record.                                                                                                                      |
| `joinThinking`             | function  |                                                                                      | Joins a call's reasoning carriers — the splitter's separated in-content spans and the accumulated wire-side `message.thinking` — into the result's `thinking`.              |
| `extractUsage`             | function  |                                                                                      | Extracts the token usage of one wire record.                                                                                                                                |
| `extractTools`             | function  |                                                                                      | Extracts the tool calls of one wire record's `message.tool_calls`.                                                                                                          |
| `extractArguments`         | function  |                                                                                      | Extracts a wire `arguments` value as a record.                                                                                                                              |
