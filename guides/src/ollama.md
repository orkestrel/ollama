...
- [`tests/src/server/officialParity.test-d.ts`](../../tests/src/server/officialParity.test-d.ts) — a compile-time drift gate asserting our wire request/response shapes stay compatible with the official `ollama` client's types (checked by `npm run check`, no daemon).
