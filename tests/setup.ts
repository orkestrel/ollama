import { AgentJobInput, ContextFormatInterface, ConversationSnapshot, ConversationStoreInterface, ConversationSummarizer, MessageInterface, ProviderDelta, ProviderInterface, ProviderResult, ProviderStreamOptions, ToolCall, ToolDefinition, ToolInterface, WorkspaceSnapshot, WorkspaceStoreInterface } from '@orkestrel/agent'
import { TokenUsage } from '@orkestrel/budget'
import { LogLevel, SinkInterface } from '@orkestrel/console'
import { FieldPath, Guard } from '@orkestrel/contract'
import { AggregateFunction, Columns, Condition, Connector, ConditionOperator, Criteria, DriverInterface, Key, Row, RowOf, TableInterface, TableSchema } from '@orkestrel/database'
import { EmitterInterface, EventMap } from '@orkestrel/emitter'
import { MarkdownInterface, BlockNode, HeadingNode, isHeadingNode, ListNode, isListNode, TableNode, isTableNode, ParagraphNode, isParagraphNode, CodeBlockNode, isCodeBlockNode, BlockquoteNode, isBlockquoteNode, InlineNode, EmphasisNode, isEmphasisNode, CodeSpanNode, isCodeSpanNode, LinkNode, isLinkNode, isTextNode, createMarkdown } from '@orkestrel/markdown'
import { InferentialResult, LogicalResult, QuantitativeDefinition, QuantitativeResult, ReasonerInterface, Reasoning, ReasonResult, Subject, SymbolicResult } from '@orkestrel/reason'
import { SSEEvent } from '@orkestrel/sse'
import { SchedulerInterface, SchedulerOptions, WorkflowDefinition, WorkflowFunction, WorkflowSnapshot } from '@orkestrel/workflow'
import { createBinaryContent, createConversation, createFile, createTextContent, createTool, createWorkspace, ProviderAbortError } from '@orkestrel/agent'
import { integerShape, isArray, stringShape } from '@orkestrel/contract'
import { createDatabase, createMemoryDriver } from '@orkestrel/database'
import { factorGroup, quantitativeDefinition, staticFactor } from '@orkestrel/reason'
import { belongsTo, hasMany } from '@orkestrel/relation'
import { createSSEParser } from '@orkestrel/sse'
import { createWorkflowRunner } from '@orkestrel/workflow'
import { describe, expect, it } from 'vitest'

/** Test-only principal shape for app-layer identity / capability fixtures. */
...
/** Test-only audit-record shape for governance / provenance fixtures. */
...
/** Test-only JSON-RPC request shape for MCP transport fixtures. */
...
/** Injectable timer callback registration used by manual timer test helpers. */
export type TimerCancel = () => void

/** Injectable timer handler seam for hermetic prompt / reconnect tests. */
export type TimerHandler = (callback: () => void) => TimerCancel
...
/**
 * Stand up a LIVE, seeded `users` {@link import('@orkestrel/agent').TableInterface} for the `databases`
...
/**
 * Create a recorder for an {@link import('@orkestrel/agent').EmitterErrorHandler} — the emitter's
...
/**
 * A recording {@link import('@orkestrel/agent').SinkInterface} — a real `SinkInterface` whose `write`
 ...
 * stream an injected client `fetch` returns so a {@link import('@orkestrel/agent').PromptClientInterface}
...
export function firstBlock(markdown: string): BlockNode {
	const block = createMarkdown(markdown).document.children[0]
...
export function assertInlineNode(parser: MarkdownInterface, markdown: string): InlineNode {
