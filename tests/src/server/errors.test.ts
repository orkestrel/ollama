import type { OllamaErrorOptions } from '@src/server'
import { isOllamaHTTPError, OllamaHTTPError } from '@src/server'
import { describe, expect, it } from 'vitest'

describe('OllamaHTTPError', () => {
	// The constructor's third parameter is the named, exported `OllamaErrorOptions`, so a
	...
	it('accepts a declared OllamaErrorOptions and keeps its cause', () => {
		const transport = new TypeError('fetch failed')
		const options: OllamaErrorOptions = { cause: transport }
