...
// real `@orkestrel/agent` consumer, plus the compile-time contract it shares with the
// official `ollama` client. Every runtime proof here is hermetic — a recording proxy
// captures the exact `/api/chat` request body before a deliberately unreachable forward
// fails — so the whole file passes with the daemon down. The module tests for the
// provider and its factory stay in `OllamaProvider.test.ts` and `factories.test.ts`;
// this file proves what a real consumer's context assembly puts on the wire.
...
// --- Compile-time parity with the official ollama client ---------------------
